...
# ─── BRAIN COMPRESSION ────────────────────────────────────────────────────────
# Embedded compression for backup/restore — no separate file needed.
# Brain files are ~80% smaller. All data is lossless / fully reversible.
...
_BC_TIER_ENC = {"STRONG":"S","MODERATE":"M","WEAK":"W","MID":"m","":"-",None:"-"}
_BC_TIER_DEC = {"S":"STRONG","M":"MODERATE","W":"WEAK","m":"MID","-":None}
_BC_BLM_ENC  = {"odds":"o","fingerprint":"f","tier":"t","form":"r"}
...
def _bc_compress_match_log(ml):
    if not ml: return {"_c":1,"teams":[],"seasons":[],"base_rid":0,"rows":[]}
    tm,sm,teams_l,seasons_l = {},{},{},[]
    # fix: use lists not dicts for teams_l/seasons_l
    teams_l = []; seasons_l = []
    def tidx(n):
        n=str(n).upper().strip()
        if n not in tm: tm[n]=len(teams_l); teams_l.append(n)
        return tm[n]
    def sidx(s):
        s=str(s)
        if s not in sm: sm[s]=len(seasons_l); seasons_l.append(s)
        return sm[s]
    rids=[e.get("round_id",0) for e in ml if e.get("round_id",0)>0]
    base=min(rids) if rids else 0
    rows=[]
    for e in ml:
        rows.append((tidx(e.get("home","?")),tidx(e.get("away","?")),
                     int(e.get("score_h") or 0),int(e.get("score_a") or 0),
...
def _bc_decompress_match_log(c):
    ...
    if not isinstance(c, dict) or "_c" not in c: return c if isinstance(c,list) else []
    teams=c.get("teams",[]); seasons=c.get("seasons",[]); base=c.get("base_rid",0)
    out=[]
    for row in c.get("rows",[]):
        try:
            h,a,sh,sa,rd,si=row
            out.append({"home":teams[h] if h<len(teams) else "?",
...
def _bc_compress_fp_rec(r):
    ...
    fp=r.get("fp_key") or []
    fp_ints=[int(round(float(v)*100)) for v in fp]
    def enc_form(f):
    ...
    return (fp_ints,
            ...
            enc_form(r.get("_form_h")),
            enc_form(r.get("_form_a")),
            round(float(r.get("dominant_outcome_conf",1.0)),3),
            _bc_enc_htft(r.get("dominant_htft","")),
            round(float(r.get("dominant_htft_conf",1.0)),3),
            int(r.get("n_samples",1)),
            _BC_OUT_ENC.get(r.get("dominant_outcome"),"?"))

def _bc_decompress_fp_rec(t):
    if isinstance(t,dict): return t
    if not isinstance(t,tuple) or len(t)<25: return {}
    (fp_ints,out,htft,btts,ou15,ou25,ou35,sh,sa,hth,hta,dc,rid,sid,lid,
     flip,pos_h,pos_a,fh_enc,fa_enc,dom_conf,dom_htft,dom_htft_conf,
     n_samp,dom_out)=t[:25]
    def dec_form(enc):
        if not enc: return {}
    ...
    return {
        "fp_key":               [v/100.0 for v in fp_ints],
        "odds_snapshot":        {},
        ...
        "round_id":             rid,"season_id":sid,"league_id":lid,
        "was_flipped":          bool(flip),
        "pos_h":                None if pos_h==-1 else pos_h,
        "pos_a":                None if pos_a==-1 else pos_a,
        "_form_h":              dec_form(fh_enc),
        "_form_a":              dec_form(fa_enc),
        "dominant_outcome":     _BC_OUT_DEC.get(dom_out,dom_out),
        "dominant_outcome_conf":dom_conf,
        "dominant_htft":        _bc_dec_htft(dom_htft),
        "dominant_htft_conf":   dom_htft_conf,
        "n_samples":            n_samp,
    }

def _bc_compress_fp_db(fp_db):
    return {fk:[_bc_compress_fp_rec(r) for r in recs] if isinstance(recs,list) else recs
            for fk,recs in fp_db.items()}

def _bc_decompress_fp_db(fp_db):
    return {fk:[_bc_decompress_fp_rec(r) if isinstance(r,tuple) else r for r in recs]
...
def _bc_compress_pm(pm):
    ...
    for fk,rec in pm.items():
        if not isinstance(rec,dict): out[fk]=rec; continue
        nr=dict(rec)
        ...
        if "results" in nr and isinstance(nr["results"],list):
            nr["results"]="|".join(f"{int(r[0])},{int(r[1])}"
                                   for r in nr["results"] if len(r)>=2)
        out[fk]=nr
    return out
...
def _bc_compress_fm_rec(r):
    if not isinstance(r,dict): return r
    blame=[_BC_BLM_ENC.get(b,b[0] if b else "") for b in (r.get("blame") or [])]
    return (int(r.get("round",0)),
            _BC_OUT_ENC.get(r.get("predicted"),"?"),
            _BC_OUT_ENC.get(r.get("actual"),"?"),
            int(r.get("score_h",0) or 0),
            int(r.get("score_a",0) or 0),
            1 if r.get("correct") else 0,
            _BC_OUT_ENC.get(r.get("odds_tip"),"?"),
            _BC_OUT_ENC.get(r.get("fp_tip"),"?"),
            _BC_OUT_ENC.get(r.get("form_tip"),"?"),
            _BC_TIER_ENC.get(r.get("h_tier"),"-"),
            _BC_TIER_ENC.get(r.get("a_tier"),"-"),
            str(r.get("tier_pair",""))[:20],
            _BC_TIER_ENC.get(r.get("strong_side"),"-"),
            "".join(blame),
            str(r.get("primary_blame",""))[:1],
            1 if r.get("btts") else 0,
            1 if r.get("over25") else 0,
            round(float(r.get("btts_prob",0) or 0),1),
            round(float(r.get("over25_prob",0) or 0),1),
            int(r.get("h_pos") or 0),
            int(r.get("a_pos") or 0),
            int(r.get("h_pts") or 0),
            int(r.get("a_pts") or 0),
            str(r.get("recovery_signal") or "")[:3],
            1 if r.get("recovery_correct") else 0,
            round(float(r.get("recovery_strength",0) or 0),2),
            round(float(r.get("confirm_rate",0) or 0),2))

def _bc_decompress_fm_rec(t):
    if isinstance(t,dict): return t
    if not isinstance(t,tuple) or len(t)<27: return {}
    _BLM_DEC={"o":"odds","f":"fingerprint","t":"tier","r":"form"}
    (rnd,pred,actual,sh,sa,correct,odds_tip,fp_tip,form_tip,
     h_tier,a_tier,tier_pair,strong_side,blame_str,primary_blame,
     btts,over25,btts_prob,over25_prob,h_pos,a_pos,h_pts,a_pts,
     rec_sig,rec_cor,rec_str,conf_rate)=t[:27]
    blame=[_BLM_DEC.get(c,c) for c in blame_str]
    _PRIMARY={"o":"odds","f":"fingerprint","t":"tier","r":"form"}
    return {
        "round":              rnd,
        "predicted":          _BC_OUT_DEC.get(pred,pred),
        "actual":             _BC_OUT_DEC.get(actual,actual),
        "score_h":            sh,"score_a":sa,
        "correct":            bool(correct),
        "odds_tip":           _BC_OUT_DEC.get(odds_tip),
        "fp_tip":             _BC_OUT_DEC.get(fp_tip),
        "form_tip":           _BC_OUT_DEC.get(form_tip),
        "h_tier":             _BC_TIER_DEC.get(h_tier,h_tier),
        "a_tier":             _BC_TIER_DEC.get(a_tier,a_tier),
        "tier_pair":          tier_pair,
        "strong_side":        _BC_TIER_DEC.get(strong_side),
        "blame":              blame,
        "primary_blame":      _PRIMARY.get(primary_blame,""),
        "btts":               bool(btts),
        "over25":             bool(over25),
        "btts_prob":          btts_prob,
        ...
        "recovery_signal":    rec_sig or None,
        "recovery_correct":   bool(rec_cor),
        "recovery_strength":  rec_str,
...
def _bc_compress_fixture_mem(fm):
    return {fk:[_bc_compress_fm_rec(r) for r in recs] if isinstance(recs,list) else recs
            for fk,recs in fm.items()}

def _bc_decompress_fixture_mem(fm):
    return {fk:[_bc_decompress_fm_rec(r) if isinstance(r,tuple) else r for r in recs]
            if isinstance(recs,list) else recs
            for fk,recs in fm.items()}

def _bc_compress_ai_log(al):
    out=[]
    for r in al:
        if not isinstance(r,dict): out.append(r); continue
        out.append((int(r.get("round",0)),
                    str(r.get("match",""))[:12],
                    _BC_OUT_ENC.get(r.get("predicted"),"?"),
                    _BC_OUT_ENC.get(r.get("actual"),"?"),
                    str(r.get("score",""))[:5],
...
def _bc_decompress_ai_log(al):
    _PRIMARY={"o":"odds","f":"fingerprint","t":"tier","r":"form"}
    out=[]
    for r in al:
        if isinstance(r,dict): out.append(r); continue
        if isinstance(r,tuple) and len(r)>=7:
            out.append({"round":r[0],"match":r[1],
                        "predicted":_BC_OUT_DEC.get(r[2],r[2]),
                        "actual":_BC_OUT_DEC.get(r[3],r[3]),
                        "score":r[4],"primary_blame":_PRIMARY.get(r[5],""),
                        "tier_pair":r[6],"blame":[],"diagnosis":""})
    return out

def _bc_compress_brain(brain_data: dict) -> dict:
    """Compress one league's brain dict before sending as backup file."""
    b=dict(brain_data)
    if "match_log" in b and isinstance(b["match_log"],list):
        b["match_log"]=_bc_compress_match_log(b["match_log"])
    if "fingerprint_db" in b and isinstance(b["fingerprint_db"],dict):
        b["fingerprint_db"]=_bc_compress_fp_db(b["fingerprint_db"])
    if "pattern_memory" in b and isinstance(b["pattern_memory"],dict):
        b["pattern_memory"]=_bc_compress_pm(b["pattern_memory"])
    if "ai_brain" in b and isinstance(b["ai_brain"],dict):
        ai=dict(b["ai_brain"])
        if "fixture_mem" in ai and isinstance(ai["fixture_mem"],dict):
            ai["fixture_mem"]=_bc_compress_fixture_mem(ai["fixture_mem"])
        if "ai_log" in ai and isinstance(ai["ai_log"],list):
            ai["ai_log"]=_bc_compress_ai_log(ai["ai_log"])
        b["ai_brain"]=ai
    b["_brain_compressed"]=1
    return b

def _bc_decompress_brain(brain_data: dict) -> dict:
    """Decompress a league brain dict after loading from backup file."""
    if not isinstance(brain_data,dict): return brain_data
    if not brain_data.get("_brain_compressed"): return brain_data
    b=dict(brain_data)
    if "match_log" in b and isinstance(b["match_log"],dict):
        b["match_log"]=_bc_decompress_match_log(b["match_log"])
    if "fingerprint_db" in b and isinstance(b["fingerprint_db"],dict):
        b["fingerprint_db"]=_bc_decompress_fp_db(b["fingerprint_db"])
    if "pattern_memory" in b and isinstance(b["pattern_memory"],dict):
        b["pattern_memory"]=_bc_decompress_pm(b["pattern_memory"])
    if "ai_brain" in b and isinstance(b["ai_brain"],dict):
        ai=dict(b["ai_brain"])
        if "fixture_mem" in ai and isinstance(ai["fixture_mem"],dict):
            ai["fixture_mem"]=_bc_decompress_fixture_mem(ai["fixture_mem"])
        if "ai_log" in ai and isinstance(ai["ai_log"],list):
            ai["ai_log"]=_bc_decompress_ai_log(ai["ai_log"])
        b["ai_brain"]=ai
    del b["_brain_compressed"]
    return b
...
def _ml_weights(bot_data: dict) -> dict:
    ...
    if "ml_weights" not in bot_data:
        bot_data["ml_weights"] = {
            "1x2":  {"correct": 1, "total": 2},
            "ou":   {"correct": 1, "total": 2},
            "btts": {"correct": 1, "total": 2},
...
def _risk_control(bot_data: dict) -> dict:
    ...
    bot_data["risk"].setdefault("loss_streak", 0)
...
def multi_market_predict(odds: dict, bot_data: dict | None = None) -> dict:
    ...
    if o1x2:
        try:
            ...
            if total > 0:
                wt = w("1x2")
                score["HOME"] += (h / total) * wt
                score["DRAW"] += (d / total) * wt
                score["AWAY"] += (a / total) * wt
                used.append("1x2")
        except (TypeError, ValueError, ZeroDivisionError):
            pass
    ...
    if htft:
        try:
            ...
            wt   = w("htft")
            mapping = {
                "1/1": ("HOME", wt),       "2/2": ("AWAY", wt),
                "X/X": ("DRAW", wt),       "1/X": ("DRAW", wt * 0.67),
                "X/1": ("HOME", wt * 0.67),"2/X": ("DRAW", wt * 0.67),
                "X/2": ("AWAY", wt * 0.67),"1/2": ("AWAY", wt * 0.50),
                "2/1": ("HOME", wt * 0.50),
            }
            out, pts = mapping.get(best, (None, 0))
            if out:
                score[out] += pts
                used.append("htft")
        except (TypeError, ValueError, StopIteration):
    ...
    if dc:
        try:
            best_dc = min(dc.items(), key=lambda x: x[1])[0]
            wt = w("dc")
            if best_dc == "1X":
                score["HOME"] += wt / 2
            elif best_dc == "X2":
...
def update_market_learning(bot_data: dict, predicted: str, actual: str, odds: dict) -> None:
    ...
    def _update(key: str, correct: bool) -> None:
        weights[key]["total"] += 1
        if correct:
    ...
    if dc:
        try:
            best = min(dc.items(), key=lambda x: x[1])[0]
            if best == "1X":
                _update("dc", actual in ("HOME", "DRAW"))
            elif best == "X2":
...
# ── Trajectory Confirmation Layer ────────────────────────────────────────────

def trajectory_confirmation(team_matches: list, current_opp_pos: int,
                             cycle_type: str, model: dict | None = None) -> dict:
    """
    Validates a cycle signal (strong_lost / weak_won) against the team's
    actual performance trajectory over the last 6 matches.

    cycle_type: "strong_lost" or "weak_won"
    team_matches: list of dicts with keys team_pos, opp_pos, goals_for, goals_against
    model: optional — used to apply self-learned accuracy boost/penalty

    Returns {"confirm": bool, "confidence_boost": int, "reason": str}
    """
    ...
    for match in team_matches[-6:]:
        team_pos = match.get("team_pos", 10)
        opp_pos  = match.get("opp_pos",  10)
        ...
        if gf > ga:   result = "win"
        elif gf == ga: result = "draw"
        else:          result = "loss"

        if position_diff > 0:   # team is stronger by position
            score = 1 if result == "win" else (0 if result == "draw" else -2)
        else:                    # team is weaker / equal
            score = 2 if result == "win" else (1 if result == "draw" else -1)
    ...
    # Trend: sum of deltas across the sequence
    ...
    # ── Self-learned accuracy adjustment (Phase 2) ────────────────────────────
    if model is not None:
        stats = model.get("trajectory_stats", {}).get(cycle_type, {})
        total = stats.get("total", 0)
        if total >= 10:
            ...
            if accuracy < 0.55:
                ...
                boost += 10   # trajectory has been reliable — strengthen it

    return {"confirm": confirm, "confidence_boost": boost, "reason": reason}


def _update_trajectory_stats(model: dict, cycle_type: str,
                               prediction_correct: bool) -> None:
    """
    Called after a result is known. Updates the self-learning trajectory stats
    so trajectory_confirmation() can adjust its influence over time.

    cycle_type: "strong_lost" or "weak_won"
    prediction_correct: True if the prediction that used this trajectory was right
    """
    stats = model.setdefault("trajectory_stats", {
        "strong_lost": {"correct": 0, "total": 0},
        "weak_won":    {"correct": 0, "total": 0},
    })
    stats.setdefault(cycle_type, {"correct": 0, "total": 0})
    stats[cycle_type]["total"] += 1
    if prediction_correct:
        stats[cycle_type]["correct"] += 1


def _build_trajectory_matches(team: str, model: dict,
                                standings: dict) -> tuple[list, int]:
    """
    Builds the team_matches list and current_opp_pos value required by
    trajectory_confirmation(), derived from match_log and standings.

    Returns (team_matches, current_opp_pos).
    team_matches entries: {team_pos, opp_pos, goals_for, goals_against}
    """
    games     = _team_last6(team, model, n=6)
    tier_map  = _get_all_tiers(standings) if standings else {}

    def _pos(name: str) -> int:
        """Approximate position from standings or tier (1=top, 20=bottom)."""
        row = _find_in_standings(name, standings) if standings else None
        if row and row.get("pos"):
            return row["pos"]
        # Fallback: tier → rough position
        tier = _find_tier(name, tier_map)
        return {"STRONG": 3, "MODERATE": 12, "WEAK": 18, "MID": 9}.get(tier, 10)
    ...
    for g in games:
        ...
        matches.append({
            "team_pos":      team_pos,
            "opp_pos":       opp_pos,
            "goals_for":     g.get("gf", 0),
            "goals_against": g.get("ga", 0),
        })

    # current_opp_pos will be supplied by the caller per-match
    return matches, 10   # 10 = neutral fallback
...
def elite_strategy_predict(home: str, away: str, odds: dict, model: dict,
    ...
    # ── GATE 2b: Trajectory Confirmation ──────────────────────────────────────
    # Validate the cycle signal against the triggering team's actual form trajectory.
    # If the team's trajectory contradicts the expected pattern, hard-reject.
    _traj_team = home if predicted_side == "HOME" else away
    _traj_opp  = away if predicted_side == "HOME" else home
    _traj_opp_row = _find_in_standings(_traj_opp, standings) if standings else None
    _traj_opp_pos = _traj_opp_row.get("pos", 10) if _traj_opp_row else 10

    _traj_matches, _ = _build_trajectory_matches(_traj_team, model, standings)
    _cycle_type_map  = {"STRONG_LOSS": "strong_lost", "WEAK_WIN": "weak_won"}
    _traj_cycle_type = _cycle_type_map.get(strategy_type, "strong_lost")

    _traj_result = trajectory_confirmation(
        team_matches     = _traj_matches,
        current_opp_pos  = _traj_opp_pos,
        cycle_type       = _traj_cycle_type,
        model            = model,
    )
    if not _traj_result["confirm"]:
        log.info(
            f"🚫 elite_strategy: trajectory REJECTED [{LEAGUES.get(league_id, {}).get('name','')}] "
            f"{home} vs {away} reason={_traj_result['reason']}"
        )
        return None

    _traj_boost = _traj_result["confidence_boost"]
    ...
    # Apply trajectory confirmation boost/penalty
    confidence = round(min(97.0, max(0.0, confidence + _traj_boost)), 2)
            ...
            for round_id, pred_entry in list(rounds.items()):
                try:
                    ...
                    for _res in results:
                        ...
                        # ── Trajectory self-learning update ────────────────────
                        _traj_cycle_key = {"STRONG_LOSS": "strong_lost", "WEAK_WIN": "weak_won"}.get(_e_stype)
                        if _traj_cycle_key:
                            _update_trajectory_stats(_elite_model, _traj_cycle_key, _e_correct)
...
async def _do_backup_inner(message, c):
    ...
    # ── Compress brain data before chunking (~80% smaller files) ────────────
    brain_data = {lid: _bc_compress_brain(lbrain) for lid, lbrain in brain_data.items()}
...
async def _load_brain_from_file_id(file_id: str, bot) -> dict | None:
    ...
    try:
        ...
        if "brain" not in data:
            ...
            return None
        # Decompress if this is a compressed brain file
        brain = {lid: _bc_decompress_brain(lbrain) for lid, lbrain in data["brain"].items()}
        log.info(f"✅ Brain file loaded: date={data.get('backup_date')} leagues={len(brain)}")
        return brain
    except Exception as e:
...
async def _apply_backup_to_bot(data: dict, message, bot_data: dict):
    ...
    if "risk" in data and data["risk"]:
        ...
        log.info(f"✅ Restored risk/loss_streak from backup")
